...
###########################   loading files    #################################
data_1 = pd.read_csv('./Ru/data/FuturesData/ru201401m.csv',encoding='GB18030', 
                     usecols=range(2,6)+[7]+range(12,16))
#pylab.plot(data_1)
data_2 = pd.read_csv('./Ru/data/FuturesData/ru201402m.csv',encoding='GB18030', 
                     usecols=range(2,6)+[7]+range(12,16))
#pylab.plot(data_2)
data_3 = pd.read_csv('./Ru/data/FuturesData/ru201403m.csv',encoding='GB18030', 
                     usecols=range(2,6)+[7]+range(12,16))
#pylab.plot(data_3)
data_4 = pd.read_csv('./Ru/data/FuturesData/ru201404m.csv',encoding='GB18030', 
                     usecols=range(2,6)+[7]+range(12,16))
data_5 = pd.read_csv('./Ru/data/FuturesData/ru201405m.csv',encoding='GB18030', 
                     usecols=range(2,6)+[7]+range(12,16))
data_6 = pd.read_csv('./Ru/data/FuturesData/ru201406m.csv',encoding='GB18030', 
                     usecols=range(2,6)+[7]+range(12,16))
data_7 = pd.read_csv('./Ru/data/FuturesData/ru201407m.csv',encoding='GB18030', 
                     usecols=range(2,6)+[7]+range(12,16))
raw_data = pd.concat([data_1, data_2, data_3,data_4,data_5,data_6, data_7], ignore_index=True)
del data_1, data_2, data_3, data_4, data_5, data_6, data_7
###################   month price plot    ####################
#pylab.plot(data_1[u'最新'])
#pylab.title('Price of Jan')
#pylab.savefig('./Ru/fig/Jan.jpg', dpi = 600)

####################    some more test data   ####################
raw_data = pd.read_csv('./Ru/data/FuturesData/ru201406m.csv',encoding='GB18030', 
                     usecols=range(2,6)+[7]+range(12,16))
...
t1 = time.clock()
raw_data["Time"] = pd.to_datetime(raw_data["Time"])      ##  about 1 minutes
t2 = time.clock()
print t2 - t1
###################   delete zero-ask days   ################################
t1 = time.clock()
zero_index = [i for i in xrange(len(raw_data)) if raw_data['askVOL1'][i] == 0 or raw_data['bidVOL1'][i] == 0]
...
t1 = time.clock()
has_zero_index = [i for i in xrange(len(raw_data)) if [raw_data['Time'][i].month,raw_data['Time'][i].day] in zero_tag]
t2 = time.clock()
print t2 - t1

clean_data = raw_data.drop(has_zero_index)
clean_data.index = range(len(clean_data))
#pylab.plot(clean_data.index)
del raw_data
####################
def split_day(x):
    day_index = []
...
day_index = split_day([x.day for x in clean_data['Time']])
t2 = time.clock()
print t2-t1
train_index = day_index[0:93]
test_index = day_index[93:]
...
t1 = time.clock()
[raise_index, fall_index, flat_idx] = feature(clean_data['Price'], train_index, change=14.5, lag=9)  ## about 170 seconds
t2 = time.clock()
print t2-t1
...
flat_index = random.sample(clean_idx, len(raise_index)+len(fall_index))
del clean_idx
...
#############################  Data Cleaning    ###############################
#askVOL = raw_data['askVOL1'] + raw_data['askVOL2'] + raw_data['askVOL3'] + \
#         raw_data['askVOL4'] + raw_data['askVOL5']
#bidVOL = raw_data['bidVOL1'] + raw_data['bidVOL2'] + raw_data['bidVOL3'] + \
#         raw_data['bidVOL4'] + raw_data['bidVOL5']
...
ratio = np.log(askVOL/bidVOL)
depth = askVOL + bidVOL

#test = [ratio[i] for i in fall_index]
#pylab.plot(test)
#print [max(test), min(test)]
#del test
#pylab.plot(std_spread)

eff_data = pd.concat([clean_data['Price'], std_spread, std_ave, clean_data['Volume'], ratio, depth,  ##  
                      clean_data['Position_chg']], axis = 1)  ## , raw_data['Position']
eff_data.columns = ["Price", "Spread", "Ave", "Volume", "Ration", "Depth", "Position_chg"]  ##  , "Position"

del askVOL, bidVOL, ask, bid, spread, ave, std_ave, std_spread, ratio, depth

#pylab.plot(eff_data["Price"])
####################### training data check  ###########################
...
raise_input = combine(data = eff_data, index=raise_index, lag = 9)
fall_input = combine(data = eff_data, index=fall_index, lag = 9)
flat_input = combine(data = eff_data, index=flat_index, lag = 9)
...
from keras.layers import Dense, Dropout, Activation, AutoEncoder
from keras.optimizers import SGD, Adadelta, Adagrad, RMSprop
from keras.callbacks import ModelCheckpoint, EarlyStopping, Callback, History
...
pd_input_max.to_csv('./Ru/model/input_max.csv')
pd_input_min = pd.Series(input_min)
pd_input_min.to_csv('./Ru/model/input_min.csv')

#pylab.plot(input_x[:,-8])
#pylab.savefig('./Ru/fig/position_chg_smoothed.jpg', dpi = 600)

model = sae(n_sample=np.shape(input_x)[1], hidden=[200,200], n_class=np.shape(input_y)[1])
model.train(input_x, input_y, batch_size=100, pre_epoch=50, fine_epoch=450, 
...
pylab.plot(score[:,1])
pylab.title('score 1')
pylab.savefig('./Ru/fig/score_1.jpg', dpi=600)
pylab.show()

pylab.plot(score[:,2])
pylab.title('score2')
pylab.savefig('./Ru/fig/score2.jpg', dpi=600)
pylab.show()
############################  out of sample   ############################
test_input = []
for i in xrange(len(test_index)):
    _test_input_ = combine(data = eff_data, index = xrange(test_index[i][0]+9, test_index[i][1]), lag = 9)
...
#                score(test_input[i], model_fall, fall_x_max, fall_x_min)]
    ...
    test_price = list(eff_data["Price"][(test_index[i][0]+9):(test_index[i][1]+1)])
    ...
    backtest.append([[r1,r2,r3],[f1,f2,f3], correctness])
t2 = time.clock()
print t2 - t1
...
raise_total = 0
fall_total = 0
for i in xrange(len(backtest)):
    raise_correct += backtest[i][0][0]
    fall_correct += backtest[i][1][2]
    raise_total = raise_total + backtest[i][0][0] + backtest[i][1][0]
    fall_total = fall_total + backtest[i][0][2] + backtest[i][1][2]
print float(raise_correct)/raise_total, float(fall_correct)/fall_total

f = open('./Ru/Ru_out_of_sample_one_model.txt', 'w+')
for i in xrange(len(backtest)):
    f.write(str(backtest[i]))
    f.write("\n")
...
for i in xrange(len(train_index)):
    _train_input_ = combine(data = eff_data, index = xrange(train_index[i][0]+9, train_index[i][1]), lag = 9)
...
#    score_train = model.model.predict(data_normalize(train_input[i], input_max, input_min))
    ...
    train_price = list(eff_data["Price"][(train_index[i][0]+9):(train_index[i][1]+1)])
...
f = open('./Ru/Ru_in_sample_one_model.txt', 'w+')
for i in xrange(len(backtest)):
    f.write(str(backtest[i]))
    f.write("\n")
f.write(str(float(raise_correct)/raise_total))
...
for i in xrange(len(weights)):
    ...
    pd_weight.to_csv('./Ru/model/weight_%i.csv' %i)
#    np.savetxt('./Ru/model/weight_%i.txt' %i,weights[i])
...
#        data = combine(data = eff_data, index = [i+test_index[d][0]+9], lag = 9)
        data = combine(data = eff_data, index = [i], lag = 9)
...
#                long_sell = eff_data['Price'][i+test_index[d][0]+9]
                long_sell = eff_data['Price'][i]
                long_trade.append({'direction':'long', 'open_time':clean_data['Time'][long_index[-1]], 
                                   'open_price':long_buy, 'next_tick_price':eff_data['Price'][long_index[-1]+1],
#                                   'close_time':raw_data['Time'][i+test_index[d][0]+9],
                                   'close_time':clean_data['Time'][i],
                                   'close_price':long_sell, 'profit':long_sell-long_buy})
                trade.append({'direction':'long', 'open_time':clean_data['Time'][long_index[-1]], 
                                   'open_price':long_buy, 'next_tick_price':eff_data['Price'][long_index[-1]+1],
#                                   'close_time':raw_data['Time'][i+test_index[d][0]+9],
                                   'close_time':clean_data['Time'][i],
                                   'close_price':long_sell, 'profit':long_sell-long_buy})
#            if test_price[i]-long_buy <= -loss:  ## cut-off
            if eff_data['Price'][i]-long_buy <= -loss:
                long_position = False
    #            long_sell = test_price[i]
#                long_sell = eff_data['Price'][i+test_index[d][0]+9]
                long_sell = eff_data['Price'][i]
...
#                short_sell = eff_data['Price'][i+test_index[d][0]+9]
                short_sell = eff_data['Price'][i]
                short_trade.append({'direction':'short', 'open_time':clean_data['Time'][short_index[-1]], 
                                    'open_price':short_buy, 'next_tick_price':eff_data['Price'][short_index[-1]+1],
#                                    'close_time':raw_data['Time'][i+test_index[d][0]+9],
                                    'close_time':clean_data['Time'][i],
                                    'close_price':short_sell, 'profit':short_buy-short_sell})
                trade.append({'direction':'short', 'open_time':clean_data['Time'][short_index[-1]], 
                                    'open_price':short_buy, 'next_tick_price':eff_data['Price'][short_index[-1]+1],
#                                    'close_time':raw_data['Time'][i+test_index[d][0]+9],
                                    'close_time':clean_data['Time'][i],
                                    'close_price':short_sell, 'profit':short_buy-short_sell})
            if short_buy-eff_data['Price'][i] <= -loss: ## cut-off
                short_position = False
    #            short_sell = test_price[i]
#                short_sell = eff_data['Price'][i+test_index[d][0]+9]
                short_sell = eff_data['Price'][i]
...
##########################  price in each day   ##########################
for d in xrange(len(test_index)):
    test_price = pd.Series(list(eff_data["Price"][(test_index[d][0]+9):(test_index[d][1]+1)]), 
                           index=clean_data['Time'][(test_index[d][0]+9):(test_index[d][1]+1)])
    test_price.plot()
    pylab.grid()
...
for d in xrange(len(test_index)):
    test_price = clean_data.iloc[(test_index[d][0]):(test_index[d][1]+1),[0,1]]
